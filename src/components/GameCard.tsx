      ...
      {parts.map((part, i) => {
        if (/[，。、！：？]/.test(part)) {
          return (
            <span
              key={i}
              className="font-sans text-[0.6em] align-baseline mx-[2px] opacity-60"
            >
              {part}
            </span>
          );
      ...
      >
        <div className="flex items-start justify-between mb-10 text-white">
          ...
          <motion.div className="flex items-center space-x-4">
            <span className="text-[10px] opacity-40 font-serif italic tracking-widest">
              No. 0{index + 1}
            </span>
        ...
        <div className="relative z-10 flex-grow text-left flex flex-col space-y-6">
          <div>
            <h4 className="text-2xl md:text-3xl font-serif font-light tracking-[0.15em] text-white whitespace-nowrap mb-1">
              {game.title}
            </h4>
            <h3 className="text-[10px] md:text-xs font-serif italic text-white/40 mb-2 truncate tracking-widest">
              {game.subtitle}
            </h3>
          </div>
